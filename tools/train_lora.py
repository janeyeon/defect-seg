    ...
    try:
        ...
        for idx, individual_datasets in enumerate(train_datasets):
            ...
            if cfg.TRAIN.method == "SOFS":
                ...
                ##########################################
                ############# params for LoRA ############
                LoRA_LR = cfg.TRAIN.SOFS_LoRA.lora_lr

                lora_params = []
                for name, module in model.lora_attach_layers.items():
                    lora_params += [param for param in module.lora_layer.parameters()]
            ...
            if cfg.NUM_GPUS > 1:
                ...
                model = torch.nn.parallel.DistributedDataParallel(
                    ...
                    find_unused_parameters=True,  ############ False --> True for training LoRA
                )
            else:
                # todo a single gpu
                pass
