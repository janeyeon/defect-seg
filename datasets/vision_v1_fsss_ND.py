...
class VISION_V1_FSSS_ND(BASE_DATASET_FSSS_ND):
    ...
    def __init__(
        ...
        if self.mode == "train":
            ...
            target_split_list = VISION_V1_split_train_val_dict[self.data_split]
            LOGGER.info("current val class is {}".format(target_split_list))
        else:
        ...
        mode_name = "train" if self.mode in ["train", "val"] else "test"

        #####################################################################
        os.makedirs(cfg.DATASET.vision_data_save_path, exist_ok=True)
        #####################################################################
    ...
    def get_basic_element(self, init_dir):
        ...
        for tmp_path in init_dir:
            ...
            if tmp_object in target_split_list:
                train_path = os.path.join(tmp_path, tmp_object, "train")
                val_path = os.path.join(tmp_path, tmp_object, "val")
